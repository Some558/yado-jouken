...
- ✅ **認証**: `applicationId`(UUID36字)+`accessKey`(46字トークン)の2点をクエリで渡す。**さらに `Referer` と `Origin` の両ヘッダー必須**(値=アプリ登録の Allowed websites ドメイン `https://yado-jouken.com`)。欠けると403 `REQUEST_CONTEXT_BODY_HTTP_REFERRER_MISSING`
